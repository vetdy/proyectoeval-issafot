...
function GenerarPlanillas() {
    ...
    useEffect(() => {
        const fetchEmpresas = async () => {
            try {
                ...
                if (!response.ok) {
                    throw new Error(
                        `Error al obtener las empresas: ${response.status}`
                    );
                }
    ...
    const generarPlanilla = async (empresa) => {
        if (empresa.estado === "aceptado") {
            if (empresa.planillas_creada) {
                abrirModal(
                    `La planilla para ${empresa.nombre_empresa} ya esta generada.`
                );
                return;
            }
            setGenerando(true);
            const consulta = await generarPlanillasProyectoEmpresa(
                empresa.id_proyecto_empresa
            );
            if (consulta.status === 200) {
                abrirModal(
                    `Se han generado las planillas para ${empresa.nombre_empresa}`,
                    () => {
    ...
    const generarTodas = async () => {
        const aceptadas = empresas.filter(
            (empresa) =>
        ...
        setGenerando(true);
        const consultas = aceptadas.map((a) =>
            generarPlanillasProyectoEmpresa(a.id_proyecto_empresa)
        );
        const res = await Promise.all(consultas);
        if (res.every((r) => r.status === 200)) {
            abrirModal(
                `Se han generado planillas para: ${aceptadas
                    .map((a) => a.nombre_empresa)
                    .join(", ")}`,
                () => {
                    window.location.reload();
                }
        ...
        <>
                ...
                <div className="container-fluid">
                    <div className="row">
                        <div className="col d-flex justify-content-center">
                            <IconoCargando tipo="linea" tamano="100%"/>
                        </div>
                    </div>
                </div>
            )}
            <div className="container mt-5">
                ...
                <h2 className="text-center mb-4">
                    Generar Planillas de Seguimiento y Evaluación Semanal
                </h2>
                ...
                <div className="text-center">
                    <button
                        className="btn fw-bold btn-eva-secondary"
